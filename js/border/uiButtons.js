...
class UIButton extends GameObject {
    ...
    render = () => {
        canvasManager.renderSpriteFromSheet(this.sprite, this.pos, this.width, this.height, this.spriteSheetPos.add(this.mouseHeldLeft ? 2 : this.pressedCondition() ? 4 : 0, 0));
        if (this.mouseHovering) {
            canvasManager.renderSpriteFromSheet(this.sprite, this.pos, this.width, this.height, this.spriteSheetPos.add(1 + (this.mouseHeldLeft ? 2 : this.pressedCondition() ? 4 : 0), 0));
        }
    };
}
