...
musicButton.render = () => {
  canvasManager.renderSpriteFromSheet(
    ...
    16,
    16,
    musicButton.spriteSheetPos.add(
      musicButton.mouseHeldLeft ? 2 : soundManager.mutedMusic ? 0 : 4,
      0,
    ),
  );
  if (musicButton.mouseHovering) {
    canvasManager.renderSpriteFromSheet(
      ...
      musicButton.height,
      musicButton.spriteSheetPos.add(
        1 + (musicButton.mouseHeldLeft ? 2 : soundManager.mutedMusic ? 0 : 4),
        0,
      ),
    );
  }
...
sfxButton.render = () => {
  canvasManager.renderSpriteFromSheet(
    ...
    16,
    16,
    sfxButton.spriteSheetPos.add(
      sfxButton.mouseHeldLeft ? 2 : soundManager.mutedSfx ? 0 : 4,
      0,
    ),
  );
  if (sfxButton.mouseHovering) {
    canvasManager.renderSpriteFromSheet(
      ...
      sfxButton.height,
      sfxButton.spriteSheetPos.add(
        1 + (sfxButton.mouseHeldLeft ? 2 : soundManager.mutedSfx ? 0 : 4),
        0,
      ),
    );
  }
